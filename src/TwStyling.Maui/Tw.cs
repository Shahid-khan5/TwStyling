...
/// </code>
/// with <c>xmlns:tw="clr-namespace:TwStyling.Maui;assembly=TwStyling.Maui"</c> (or the URL form
...
public static class TwRuntime
{
    ...
    public static TwEngine Engine
    {
        get
        {
            ...
            lock (InitLock)
            {
                if (_engine is null)
                {
                    _engine = new TwEngine(DetectEnvironment(), ReportDiagnostic)
                    {
                        WarnOnRuntimeParse = WarnOnRuntimeParse,
                        Stylesheet = LoadEmbeddedStylesheet(),
                    };
    ...
    {
        // The resource lives in the app assembly, not this one: it is the app's stylesheet.
        foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
        {
            Stream? stream;
            try
            {
                stream = assembly.GetManifestResourceStream("Tw.tailwind.css");
            }
            catch
            {
                continue; // dynamic or otherwise unreadable assembly
            }
            ...
            try
            {
                ...
                using (var reader = new StreamReader(stream))
                    return TwCssPlanCompiler.FromCss(reader.ReadToEnd());
            }
            catch (Exception ex)
            {
                // A malformed stylesheet must not take the app down — fall back to the parser.
                ReportDiagnostic(new TwDiagnostic("", "", $"embedded Tailwind stylesheet could not be read: {ex.Message}"));
                return null;
            }
        }
        return null;
